...
concept CIsMapping = requires(TMapping mapping) {
    { *(++mapping.begin() != mapping.end() ? mapping.begin() : mapping.end()) } -> std::pair<typename TMapping::key_type, typename TMapping::mapped_type>;
};

// - print functions - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 

template<CIsIterable TIterable>
void PrintIterable(std::ostream &out, const TIterable &values, char openChar = '(', char closeChar = ')', char delimChar = ',') {
    out << openChar;
...
template<typename TIterable>
    requires CIsIterable<TIterable> && !CIsMapping<TIterable> && !std::is_base_of<std::string, TIterable>::value
